...
| KAFKA_DISCOVERY_TOPIC                    | ${ENV}.discovery             |  false   | Kafka topic the module bootstrap cache-invalidation consumer subscribes to for discovery change events. Defaults to the environment-specific `discovery` topic.                                             |
| BOOTSTRAP_CACHE_ENABLED                  | true                         |  false   | Enables the per-replica module bootstrap cache. Forced off (no-op) when `FAR_MODE` is true.                                                                                                                |
| BOOTSTRAP_CACHE_MAX_SIZE                 | 1000                         |  false   | Maximum number of cached module bootstrap snapshots (Caffeine `maximumSize`).                                                                                                                              |
| BOOTSTRAP_CACHE_TTL                      | 30m                          |  false   | Backstop expiry for a cached snapshot (Caffeine `expireAfterWrite`); freshness is otherwise driven by discovery events.                                                                                     |
| KAFKA_BOOTSTRAP_CACHE_GROUP_ID           | ${ENV}-mgr-applications-bootstrap-cache | false | Consumer group prefix for the broadcast bootstrap-cache invalidation consumer; a unique suffix is appended per instance so every replica receives every discovery event.                          |
